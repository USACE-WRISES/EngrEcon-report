...
```{r echo=TRUE}
effective_interest_rate(0.06, m = "yearly")
```

The present.to.future can be used to calculate the amount of money based on the calculated effective interest rate.

```{r echo=TRUE}
present_to_future(0.06,1,50000)
```


## Interest During Construction

`interest.during.construction`: determines interest during construction (IDC) based on the interest rate, duration, and capital investment. IDC is not a financial cost for which no one will be required to pay (Engineer & Resources, 1993). The calculation depends on the construction duration.
...
```{r echo=TRUE}
IDC <- interest_during_construction(0.0275,24,800000)
IDC
```
...
Solution using the functions in the package:
```{r echo=TRUE}
GAE <- gradient_to_annual(0.04,15,200,800)
GAE
```
...
Solution using the functions in the package:
```{r echo=TRUE}
PVB <- annual_to_present(0.04,20,500)# calculates the present value of the benefit
PVB
```
